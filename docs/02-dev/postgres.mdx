# Postgres



## Table Partitioning

- [Table Partitioning Docs](https://www.postgresql.org/docs/current/ddl-partitioning.html)



## Data Types


### JSONB

[PGConf NYC 2021 - Understanding of Jsonb Performance by Oleg Bartunov](https://www.youtube.com/watch?v=v_s-DH4PEVA)


## Foreign Data Wrappers

### Parquet

- [Parquet S3](https://github.com/pgspider/parquet_s3_fdw)
- [Parquet and Postgres in the Data Lake](https://www.crunchydata.com/blog/parquet-and-postgres-in-the-data-lake)



## Export

### Postgres to Parquet

- [connector-x](https://github.com/sfu-db/connector-x)

[Using BigQuery on GCP](https://stackoverflow.com/questions/66469369/export-gcp-cloud-sql-postgresql-to-gcs-in-parquet-format)



## Extensions

https://github.com/tcdi/pgx

https://supabase.com/blog/choosing-a-postgres-primary-key